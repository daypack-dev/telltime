...
let run (search_years_ahead : int) (time_slot_count : int) (time_format : time_format)(expr : string) :
  unit =
  let search_param =
  ...
  | Ok expr -> (
      ...
      | Ok s -> (
          (match time_format with
           | `Plain_human_readable -> Printf.printf "Time zone offset in seconds: %d\n" Config.tz_offset_s
           | `Plain_unix_second -> ()
          );
          match s () with
