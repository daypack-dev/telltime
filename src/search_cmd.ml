...
let run (tz_name : string option) (search_years_ahead : int) (time_slot_count : int)
    (sep : string) (expr : string) : unit =
  ...
  | Ok timere -> (
      ...
      let search_end_exc =
        Int64.add
          (Timere.Duration.(make ~days:(search_years_ahead * 365) () |> to_seconds))
          search_start
      in
      let timere =
        let open Timere in
